...
class TestProcessEndpoint:
    ...
    async def test_deduplication_returns_existing_job(self, client: AsyncClient, beanie_init) -> None:
    ...
    async def test_creates_job_with_correct_platform(self, client: AsyncClient, beanie_init) -> None:
        with patch("app.routers.process.check_rate_limit", new=AsyncMock(return_value=None)), \
             patch("arq.create_pool", side_effect=Exception("no redis")):
...
class TestJobsEndpoint:
    ...
    async def test_job_payload_contains_all_expected_fields(self, client: AsyncClient, beanie_init) -> None:
        job = await JobService.create("https://youtube.com/watch?v=abc", Platform.YOUTUBE)
        data = (await client.get(f"/api/jobs/{job.id}")).json()["data"]
        required_fields = {"jobId", "status", "progress", "currentStep", "progressMessage", "error", "errorCode", "completedAt"}
...
class TestTripsEndpoint:
    async def test_create_trip(self, client: AsyncClient, beanie_init) -> None:
        r = await client.post("/api/trips", json={
    ...
    async def test_update_trip_title(self, client: AsyncClient, beanie_init) -> None:
        ...
        r = await client.put(f"/api/trips/{trip.id}", json={"user_id": "clerk_upd", "title": "New Title"})
        assert r.status_code == 200
    ...
    async def test_add_pin_to_trip(self, client: AsyncClient, beanie_init) -> None:
        ...
        r = await client.post(f"/api/trips/{trip.id}/pins", json={
            "user_id": "clerk_pin",
    ...
    async def test_reorder_pins(self, client: AsyncClient, beanie_init) -> None:
        ...
        r = await client.post(f"/api/trips/{trip.id}/pins/reorder", json={
...
class TestChatEndpoint:
    async def test_chat_returns_reply(self, client: AsyncClient, beanie_init) -> None:
        ...
        r = await client.post(f"/api/trips/{trip.id}/chat", json={
