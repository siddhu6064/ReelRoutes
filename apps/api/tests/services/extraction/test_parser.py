...
class TestParseExtractionResponse:
    def test_parses_valid_json_array(self) -> None:
        raw = _json([
            {"place_name": "Shibuya Crossing", "context_quote": "We started at Shibuya", "confidence": 0.95, "timestamp_hint": 42.0},
            {"place_name": "Senso-ji Temple", "context_quote": "Then visited Senso-ji", "confidence": 0.88, "timestamp_hint": None},
        ])
    ...
    def test_strips_json_fences(self) -> None:
        raw = "```json\n" + _json([{"place_name": "Tokyo", "context_quote": "We went to Tokyo", "confidence": 0.9}]) + "\n```"
        result = parse_extraction_response(raw)
        assert result.ok
        assert len(result.locations) == 1

    def test_strips_bare_code_fence(self) -> None:
        raw = "```\n" + _json([{"place_name": "Kyoto", "context_quote": "Beautiful Kyoto", "confidence": 0.85}]) + "\n```"
        result = parse_extraction_response(raw)
        assert result.ok
    ...
    def test_filters_low_confidence(self) -> None:
        raw = _json([
            ...
            {"place_name": "Another Vague", "context_quote": "somewhere", "confidence": MIN_CONFIDENCE - 0.01},
        ])
        result = parse_extraction_response(raw)
        assert len(result.locations) == 1
    ...
    def test_keeps_exactly_at_min_confidence(self) -> None:
        raw = _json([{"place_name": "Borderline Place", "context_quote": "Borderline", "confidence": MIN_CONFIDENCE}])
        result = parse_extraction_response(raw)
        assert len(result.locations) == 1
    ...
    def test_deduplicates_by_normalised_name(self) -> None:
        raw = _json([
            {"place_name": "Shibuya Crossing", "context_quote": "First mention", "confidence": 0.8},
            {"place_name": "shibuya crossing", "context_quote": "Second mention", "confidence": 0.95},
            {"place_name": "SHIBUYA CROSSING", "context_quote": "Third mention", "confidence": 0.7},
        ])
        result = parse_extraction_response(raw)
        assert len(result.locations) == 1
...
class TestMergeChunkResults:
    ...
    def test_merge_keeps_higher_confidence_on_dedup(self) -> None:
        ...
        merged = merge_chunk_results([r1, r2])
        tokyo = next(l for l in merged.locations if l.place_name == "Tokyo")
