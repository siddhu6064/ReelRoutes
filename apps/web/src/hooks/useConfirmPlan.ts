...
export function useConfirmPlan(): ReturnType<
  typeof useMutation<PlanConfirmResponse, Error, PlanConfirmRequest>
> {
...
export function buildConfirmRequest(params: {
  ...
  return {
    ...
    days_plan: params.curatedDays.map(
      (d): ConfirmDayPlan => ({
        day: d.day,
        activity_stops: d.activityStops.map(
          (s): ConfirmActivityStop => ({
            name: s.name,
            lat: s.lat,
            lng: s.lng,
