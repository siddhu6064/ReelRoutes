...
export default function CurationScreen(): JSX.Element {
  ...
  const handleSave = (): void => {
    const req = buildConfirmRequest({
      startingPoint, destination, days, travelMode, preferences, curatedDays,
    })
    confirmPlan(req, {
      onSuccess: (res) => {
        reset()
        void navigate(`/trips/${res.trip_id}`)
      },
    })
  }
      ...
      <div className="curation-header">
        <div className="curation-header-left">
          ...
          <p className="curation-meta">
            {days} day{days !== 1 ? 's' : ''} · {totalStops} stop{totalStops !== 1 ? 's' : ''} · from {startingPoint}
          </p>
        </div>
        ...
        <div className="curation-days">
          ...
          {curatedDays.map((curatedDay, dayIndex) => (
            <DayAccordion
              ...
              onToggle={() => setOpenDay(openDay === dayIndex ? -1 : dayIndex)}
            />
          ))}
        </div>
