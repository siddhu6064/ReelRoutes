...
const ICONS: Record<string, string> = {
  restaurant:"🍽️", museum:"🏛️", park:"🌳", landmark:"🗿",
  market:"🛒", gallery:"🖼️", cafe:"☕", bar:"🍸", other:"📍",
};

export default function SpotSuggestionsSheet({
  tripId, userId, visible, onClose, onAddPin,
}: { tripId: string; userId?: string; visible: boolean; onClose: () => void; onAddPin: (s: Spot) => Promise<void>; }) {
  ...
  async function load() {
    ...
    try {
      const r = await fetch(`/api/trips/${tripId}/suggest-spots${userId ? `?user_id=${userId}` : ""}`, { method: "POST" });
      const j = await r.json();
      if (j.ok) { setSpots(j.data.suggestions); setAddedNames(new Set()); }
    ...
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      <View style={s.backdrop}>
        <View style={s.sheet}>
          ...
          {loading && <View style={s.center}><ActivityIndicator color="#6366f1" size="large" /><Text style={s.loadTxt}>Scouting nearby gems…</Text></View>}
          {error && (
            <View style={s.center}>
              <Text style={s.errTxt}>{error}</Text>
              <TouchableOpacity style={s.retry} onPress={load}><Text style={s.retryTxt}>Try again</Text></TouchableOpacity>
            </View>
          )}
          {spots.length > 0 && !loading && (
            <ScrollView contentContainerStyle={s.list}>
              <TouchableOpacity style={s.refresh} onPress={load}><Text style={s.refreshTxt}>🔄 New suggestions</Text></TouchableOpacity>
              {spots.map(spot => {
                ...
                return (
                  <View key={spot.name} style={[s.card, added && s.cardAdded]}>
                    ...
                    <View style={s.cardBody}>
                      ...
                      <Text style={s.cardAddr} numberOfLines={1}>{spot.address}</Text>
                      <Text style={s.cardReason} numberOfLines={2}>💡 {spot.reason}</Text>
                    </View>
                    <TouchableOpacity
...
const s = StyleSheet.create({
  ...
  sheet: { backgroundColor: "#fff", borderTopLeftRadius: 24, borderTopRightRadius: 24, maxHeight: "88%", paddingBottom: 32 },
  handle: { width: 36, height: 4, borderRadius: 2, backgroundColor: "#d1d5db", alignSelf: "center", marginTop: 10 },
  hdr: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", padding: 20, paddingBottom: 12 },
  ...
  card: { flexDirection: "row", alignItems: "flex-start", backgroundColor: "#fff", borderWidth: 1.5, borderColor: "#e5e7eb", borderRadius: 12, padding: 14, gap: 10 },
  ...
  addBtn: { width: 34, height: 34, borderRadius: 17, borderWidth: 2, borderColor: "#6366f1", alignItems: "center", justifyContent: "center", alignSelf: "center" },
