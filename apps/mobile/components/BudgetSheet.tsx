...
export default function BudgetSheet({
  ...
  async function handleAdd() {
    ...
    try {
      ...
      setAddErr("Could not add expense.");
    } finally {
      setAdding(false);
    }
  }
            ...
            <ScrollView contentContainerStyle={s.body} showsVerticalScrollIndicator={false}>
              ...
              {budget !== null && (
                <View style={s.trackBg}>
                  <View
                    style={[
                      s.trackFill,
                      { width: `${Math.min((total / budget) * 100, 100)}%` as unknown as number },
                    ]}
                  />
                </View>
              )}
              ...
              >
                {CATS.map((c) => (
                  <TouchableOpacity
                    key={c}
                    style={[s.catChip, cat === c && s.catChipSel]}
                    onPress={() => setCat(c)}
                  >
                    <Text style={[s.catTxt, cat === c && s.catTxtSel]}>
                      {ICONS[c]} {c}
                    </Text>
                  </TouchableOpacity>
                ))}
              ...
              <TouchableOpacity
                style={[s.addBtn, adding && s.addBtnDis]}
                onPress={handleAdd}
                disabled={adding}
                activeOpacity={0.85}
              >
